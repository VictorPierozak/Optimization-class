...
matrix::matrix(long double L)
{
	n = m = 1;
	M = new long double*[1];
	M[0] = new long double[1];
	M[0][0] = L;
}

matrix::matrix(int nv, int mv, long double L)
{
	if (nv <= 0 || mv <= 0)
		throw string("matrix::matrix(int,int,long double):\nwymiary macierzy musza byc dodatnie");
	n = nv;
	m = mv;
	M = new long double*[n];
	for (int i = 0; i < n; ++i)
	{
		M[i] = new long double[m];
		for (int j = 0; j < m; ++j)
			M[i][j] = L;
	}
}

matrix::matrix(int nv, long double* A)
{
	if (nv <= 0)
		throw string("matrix::matrix(int,long double*):\ndlugosc wektora musi byc dodatnia");
	n = nv;
	m = 1;
	M = new long double*[n];
	for (int i = 0; i < n; ++i)
	{
		M[i] = new long double[1];
		M[i][0] = A[i];
	}
}

matrix::matrix(int nv, int mv, long double** A)
{
	if (nv <= 0 || mv <= 0)
		throw string("matrix::matrix(int,int,long double**):\nwymiary macierzy musza byc dodatnie");
	n = nv;
...
long double& matrix::operator()(int nv, int mv)
{
	if (nv >= n || mv >= m || nv < 0 || mv < 0)
		throw string("long double& matrix::operator()(int,int):\nindeks jest poza zakresem");
	return M[nv][mv];
}

long double matrix::operator()(int nv, int mv) const
{
	if (nv >= n || mv >= m || nv < 0 || mv < 0)
		throw string("long double matrix::operator()(int,int) const:\nindeks jest poza zakresem");
	return M[nv][mv];
}
...
void matrix::add_col(long double L)
{
	matrix A(n, m + 1);
	for (int i = 0; i < n; ++i)
	{
...
void matrix::add_row(long double L)
{
	matrix A(n + 1, m);
	for (int i = 0; i < n; ++i)
		for (int j = 0; j < m; ++j)
...
long double m2d(const matrix& A)
{
	int* nA = get_size(A);
	if (nA[0] != 1 || nA[1] != 1)
		throw string("long double m2d(const matrix&):\nzamiana macierzy na liczbe mozliwa jest tylko dla skalarow");
	return A(0, 0);
}

long double det(const matrix& A)
{
	int* nA = get_size(A);
	if (nA[0] != nA[1])
		throw string("long double det(const matrix&):\nmacierz musi byc kwadratowa");
	long double D = 0;
	if (nA[0] == 1)
		D = A();
	else
